...
- **Integration block** (`AGENTS.md` or `CLAUDE.md`):
  - Wrapped in `<!-- krait_arch × compound-engineering integration -->` and `<!-- end krait_arch × compound-engineering integration -->` markers.
  - Has language tag (`<!-- lang: en -->` or `<!-- lang: ru -->`).
  - Workflow diagram references the current set of `/ce-*` and `/krait_arch:*` commands.
...
- **Command names** appear with `/krait_arch:` prefix unchanged: `/krait_arch:roast`, `/krait_arch:cycle`, `/krait_arch:adr`, etc.
...
**Date**: YYYY-MM-DD
**Plugin version**: <read from .krait-arch-version or plugin.json>
...
Return the meta-review as a single Markdown response. Saved by the orchestrating command (`/krait_arch:meta-review`) to `docs/architecture/reviews/YYYY-MM-DD-meta-review-<target-slug>.md`.
