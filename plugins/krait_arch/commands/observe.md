...
- Items to file as `/krait_arch:adr` for already-implicit decisions.
- Items requiring research (`/krait_arch:research`) before they can become full cycles.
...
- **Don't count `/krait_arch:upgrade` migrations as gaps.** Things that are "missing" because the project is on an older plugin version are upgrade work, not architectural gaps.
