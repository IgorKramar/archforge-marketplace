...
Run this command after `/plugin marketplace update krait-arch-marketplace` and `/plugin install krait_arch@krait-arch-marketplace` have brought your installed plugin to a new version, especially across minor versions (0.2 → 0.3, etc.).

## What it does

1. Reads the **installed plugin version** from `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json`.
2. Reads the **project's last upgrade marker** from `docs/architecture/.krait-arch-version` (a single-line file with a version string). If the file is missing, treats the project as last touched by 0.0.0.
...
5. Updates `docs/architecture/.krait-arch-version` to the installed version.
...
- **Refresh integration block in `AGENTS.md`** if present. The 0.3 block includes the `Research` phase in the workflow diagram and adds language-detection support. Run the equivalent of `/krait_arch:remember-compound-integration` and let it replace the block in place. Detect the project's working language; the new block is written in that language.
- **Add `decision-map.md` skeleton if missing**, but only if there are ≥3 open architectural questions in `ARCHITECTURE.md` or ≥2 ADRs already exist. Otherwise leave a note in chat suggesting `/krait_arch:map` as an optional next step.
...
- **Surface to user**: terminology pass for Russian artifacts is now mandatory (was advisory in 0.2). If the project has Russian artifacts, suggest re-running `/krait_arch:review` on key ADRs to apply the terminology pass.
...
```
./docs/architecture/.krait-arch-version
```

If the file doesn't exist, the project was last touched by an unknown earlier version. Treat as 0.0.0 — all migrations apply (this is the same effect as a fresh init plus all migrations).

If `docs/architecture/` itself doesn't exist, the project hasn't been initialized for krait_arch. Tell the user: "This project hasn't been initialized for krait_arch. Run `/krait_arch:init` first."
...
Write the installed version to `docs/architecture/.krait-arch-version`. This is a one-line file with just the version string and a trailing newline.
...
- Any **recommended next steps** the migrations didn't auto-apply (e.g., "we suggest running `/krait_arch:roast` on ADR-0001 since deep-cycle roast is new in 0.3").
...
- **Project not initialized**: route to `/krait_arch:init`.
...
If the marker equals the installed version: "Project is already at krait_arch <version>. Nothing to migrate. Run `/krait_arch:upgrade` again after updating the plugin."
