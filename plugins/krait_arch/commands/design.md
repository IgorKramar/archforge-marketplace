...
If no discovery document exists, briefly walk the discovery questions inline before designing — but flag this and suggest running `/krait_arch:discover` first for non-trivial problems.
...
Summarize the alternatives and the comparison matrix. Tell the user where the doc is. Suggest: "When you're ready, run `/krait_arch:decide \"<problem>\"` to commit."
