---
description: Plugin-conformance check on artifacts produced by krait_arch. Verifies templates are followed, identifiers are preserved (not translated), language pass was applied, cross-references resolve, lifecycle states are valid. Does NOT evaluate architectural quality (use /krait_arch:roast for that). Catches the plugin failing to follow its own rules.
...
# /krait_arch:meta-review

Meta-review of `krait_arch` artifacts. Reads them, compares to the plugin's own templates and rules (in command files and the `architect` skill), reports divergences.
...
- On non-`krait_arch` artifacts. The meta-reviewer doesn't lint user-authored code or unrelated docs.
...
- If it's `all`, build a list of all artifact paths under `docs/architecture/` (excluding `.krait-arch-version` and `README.md`).
...
When `/krait_arch:cycle --scale=deep` produces a roast, **automatically chain a meta-review on the roast directory** as the final step before declaring the cycle done. This catches the kind of bug v0.4.0-rc1 had — where the roast appeared to succeed but its per-role outputs were structurally divergent.
...
- Meta-review and `/krait_arch:review` (architectural code review) are also complementary. Code review looks at code; meta-review looks at architectural artifacts.
- Meta-review and `/krait_arch:diff` (planned for v0.5) will overlap slightly — diff checks whether ADR rules live in code; meta-review checks whether ADR conforms to template. Different lenses.
