...
- Suggestions for adjacent diagrams when relevant: "this container view doesn't show the deployment topology — try `/krait_arch:diagram deployment <subject>` for that."
...
The legacy `/krait_arch:c4 <level> <subject>` command still works. It is now an alias for `/krait_arch:diagram c4-<level> <subject>`. New work should use `/krait_arch:diagram` directly.
