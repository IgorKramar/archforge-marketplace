...
- **First-time architectural work on a project**, after `/krait_arch:init`, when the discovery surface is wide.
...
The router skill `architect` should also propose `/krait_arch:map` proactively when it sees a user about to start a cycle that obviously depends on other unmade decisions.
...
- A suggestion: "Run `/krait_arch:cycle \"<top item>\"` to take the first decision."
