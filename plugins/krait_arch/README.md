...
Each phase has a slash command. You can also run the whole cycle for a single problem with `/krait_arch:cycle "<problem statement>"`.
...
- **`compound-integration`** — defines how `krait_arch` interleaves with the EveryInc `compound-engineering` plugin: which phase plugs in where, who owns which artifact, how to avoid double work.
...
After `/krait_arch:init` you'll have:
...
└── docs/
    └── architecture/
        ...
        │   ├── context.md
        │   └── container.md
        ├── research/                  ← research digests from /krait_arch:discover
...
`krait_arch` is opinionated. The router skill `architect` instructs Claude to:
