...
| [`research/`](./research/) | **Discovery and design notes** from `/krait_arch:discover` and `/krait_arch:design`. Snapshots of the problem space at the time of investigation. |
| [`reviews/`](./reviews/) | **Architectural reviews** from `/krait_arch:review`. Reviews of substantial changesets, kept for posterity. |
...
- New architectural decision → `/krait_arch:cycle "<problem>"` (full process) or `/krait_arch:adr "<decision>"` (shortcut).
- Need a diagram → `/krait_arch:c4 <level> "<subject>"`.
- Architectural review of a PR or directory → `/krait_arch:review [path]`.
