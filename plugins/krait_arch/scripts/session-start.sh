...
if [ -f "$ARCH_FILE" ]; then
  cat <<EOF >&2
[krait_arch] ARCHITECTURE.md found at repo root. The architect skill treats it as binding context.
EOF
  if [ -d "$ADR_DIR" ]; then
    ADR_COUNT=$(find "$ADR_DIR" -maxdepth 1 -type f -name '[0-9]*.md' 2>/dev/null | wc -l | tr -d ' ')
    if [ "${ADR_COUNT:-0}" -gt 0 ]; then
      echo "[krait_arch] ${ADR_COUNT} ADR(s) in docs/architecture/decisions/." >&2
    fi
  fi
else
  # ARCHITECTURE.md missing. Suggest /krait_arch:init only if the repo looks
  # non-trivial (more than 20 tracked files).
  TRACKED=$(git ls-files 2>/dev/null | wc -l | tr -d ' ')
  if [ "${TRACKED:-0}" -ge 20 ]; then
    cat <<EOF >&2
[krait_arch] No ARCHITECTURE.md detected in this project. For architectural work, run /krait_arch:init
