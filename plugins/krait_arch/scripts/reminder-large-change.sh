#!/usr/bin/env bash
# krait_arch — soft architectural reminder hook.
#
...
if [ "${CHANGED_COUNT:-0}" -ge 10 ] && [ "${ADR_CHANGED:-0}" -eq 0 ]; then
  cat <<EOF >&2
[krait_arch] Reminder: ${CHANGED_COUNT} files changed in this session and no ADR has been touched.
            If this represents an architectural decision (new module, pattern, dependency, structural shift),
            consider running /krait_arch:adr to capture it. Implicit decisions become tomorrow's tech debt.
EOF
fi
...
if [ -n "$MANIFEST_CHANGED" ]; then
  cat <<EOF >&2
[krait_arch] Reminder: \`$MANIFEST_CHANGED\` was modified — possibly a new dependency.
            New dependencies are architectural decisions. If this is non-trivial (a runtime, a framework,
            a security-sensitive lib), run /krait_arch:adr to record why this dep was chosen over alternatives.
EOF
fi
...
if [ -n "$NEW_TOP_DIRS" ]; then
  while IFS= read -r dir; do
    ...
    cat <<EOF >&2
[krait_arch] Reminder: new top-level directory \`$dir/\` detected.
